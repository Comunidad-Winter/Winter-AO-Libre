...
Next LoopC
        ...
        Case "-" 'Gritar
            ...
            Exit Sub
        'Mensajes Privados por consola y Screen por Standelfito ^^
        Case "\"
             rData = Right$(rData, Len(rData) - 1)
        ...
        If Not EstaPCarea(UserIndex, tIndex) Then
            ...
            Call SendData(SendTarget.ToIndex, tIndex, 0, "||" & UserList(UserIndex).name & "> " & Arg1 & "~0~128~0~0~1")
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & UserList(UserIndex).name & "> " & Arg1 & "~0~128~0~0~1")
        Else
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "MN" & MapInfo(UserList(UserIndex).Pos.Map).name)
            ...
            If UserList(UserIndex).Counters.Saliendo Then
                Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Salida Cancelada !!!." & FONTTYPE_WARNING)
    ...
    '        Exit Sub
        ...
        Case "\?"
            Dim PuntosGanados As Integer
                PuntosGanados = GetVar(CharPath & UserList(UserIndex).name & ".Chr", "STATS", "PUNTOSTORNEO")
                    Call SendData(SendTarget.ToIndex, UserIndex, 0, "\~" & PuntosGanados)
                Exit Sub
        ...
        Case "TI" 'Tirar item
                ...
                If val(Arg1) = FLAGORO Then
                    ...
                    Call TirarOro(val(Arg2), UserIndex)
                                    If Arg2 > 50000 Then
                            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Anticheat> No Puedes Tirar esa cantidad! Utiliza el comando /Daroro NICK para entregar la cantidad deseada. " & FONTTYPE_WARNING)
                            Exit Sub
                        End If
                    Call SendUserStatsBox(UserIndex)
                    Exit Sub
