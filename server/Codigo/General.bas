...
Function HayAgua(ByVal map As Integer, ByVal X As Integer, ByVal Y As Integer) As Boolean
    ...
    If map > 0 And map < NumMaps + 1 And X > 0 And X < 101 And Y > 0 And Y < 101 Then
        If ((MapData(map, X, Y).Graphic(1) >= 1505 And MapData(map, X, Y).Graphic(1) <= 1520) Or (MapData(map, X, Y).Graphic(1) >= 5665 And MapData(map, X, Y).Graphic(1) <= 5680) Or (MapData(map, X, Y).Graphic(1) >= 13547 And MapData(map, X, Y).Graphic(1) <= 13562)) And MapData(map, X, Y).Graphic(2) = 0 Then
            HayAgua = True
        Else
            HayAgua = False
...
Sub Main()
    ...
    If Not FileExist(CharPath, vbDirectory) Then
        MkDir CharPath

    End If
    
    If Not FileExist(AccPath) Then
        MkDir AccPath
...
Public Sub RecStamina(ByVal UserIndex As Integer, _
    ...
    If UserList(UserIndex).Stats.MinSta < UserList(UserIndex).Stats.MaxSta Then
        ...
        Else
            ...
            If UserList(UserIndex).flags.Desnudo Or UserList(UserIndex).flags.Makro <> 0 Then Exit Sub 'Desnudo y trabajando no sube energia. (ToxicWaste)
...
Public Function DentroDelRango(ByVal UserIndex As Integer, _
                               ByVal NpcNumber As Integer) As Boolean
