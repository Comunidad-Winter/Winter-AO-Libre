...
Select Case Lugar
  ...
  Case Else
        ...
        End If
End Select
 'Con esto el npc le quita menos a los guerreros y cazadores.
If UserList(UserIndex).Clase = "GUERRERO" Or UserList(UserIndex).Clase = "CAZADOR" Then daño = daño / 2
...
If UserList(AtacanteIndex).flags.Invisible = 1 Then
    UserList(AtacanteIndex).flags.Invisible = 0
        Call SendData(ToMap, 0, UserList(AtacanteIndex).Pos.Map, "NOVER" & UserList(AtacanteIndex).Char.CharIndex & ",0")
    Call SendData(ToIndex, AtacanteIndex, 0, "||¡Has vuelto a ser visible!" & FONTTYPE_INFO)
End If
 
If UserList(AtacanteIndex).flags.Oculto > 0 Then
...
If ExpaDar <= 0 And ExpaDar <> -1 Then Exit Sub
