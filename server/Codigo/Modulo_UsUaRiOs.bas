...
If UserList(UserIndex).Stats.Exp = UserList(UserIndex).Stats.ELU And UserList(UserIndex).Stats.ELV = STAT_MAXELV - 1 Then
    Call SendData(SendTarget.ToAll, UserIndex, 0, "||" & UserList(UserIndex).name & " ha llegado al nivel máximo." & FONTTYPE_INFO)
    Exit Sub
End If
...
Do While UserList(UserIndex).Stats.Exp >= UserList(UserIndex).Stats.ELU
    ...
    If Not EsNewbie(UserIndex) And WasNewbie Then
        ...
        If UCase$(MapInfo(UserList(UserIndex).Pos.Map).Restringir) = "SI" Then
            ...
            Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Debes abandonar la Isla Newbie." & FONTTYPE_WARNING)
        End If
    End If
...
Call WarpUserChar(UserIndex, 70, 81, 32)
...
Dim OldY As Integer
    ...
    End If
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "MN" & MapInfo(UserList(UserIndex).Pos.Map).name)
...
Sub Cerrar_Usuario(ByVal UserIndex As Integer, Optional ByVal Tiempo As Integer = -1)
    ...
    If UserList(UserIndex).flags.UserLogged And Not UserList(UserIndex).Counters.Saliendo Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Cerrando...Se cerrará el juego en " & UserList(UserIndex).Counters.Salir & " segundos... Si te mueves se cancelara la salida." & FONTTYPE_INFO)
    End If
End Sub
...
Else
    ...
    Call SendData(SendTarget.ToIndex, sendIndex, 0, "||Puntos de torneo: " & GetVar(CharPath & Nombre & ".chr", "stats", "PuntosTorneo") & FONTTYPE_INFO)
End If
Exit Sub
