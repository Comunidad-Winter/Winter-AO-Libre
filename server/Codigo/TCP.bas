...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "CREADACUENTA")
'Call ConnectCuenta(UserIndex, Nombre, Pass)

End Sub

Sub ConnectCuenta(ByVal UserIndex As Integer, name As String, Password As String)

Dim Cuenta As Account
...
'Get char number...
Cuenta.NumChars = val(GetVar(CuentasPath & name & ".ct", "INIT", "NumPjs"))

'If we have characters then go on
If Cuenta.NumChars > 0 Then
    Dim i As Byte
    ReDim Cuenta.Chars(1 To Cuenta.NumChars)
    Dim Paquete As String

    'Read Character data
    For i = 1 To Cuenta.NumChars
        Cuenta.Chars(i).name = GetVar(CuentasPath & name & ".ct", "INIT", "PJ" & i)
        Cuenta.Chars(i).Head = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Head")
        Cuenta.Chars(i).Body = val(GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Body"))
        Cuenta.Chars(i).Helmet = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Casco")
        Cuenta.Chars(i).Shield = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Escudo")
        Cuenta.Chars(i).Weapon = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Arma")
        Cuenta.Chars(i).Status = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "REP", "Promedio")
        Cuenta.Chars(i).Pos = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "INIT", "Position")
        Cuenta.Chars(i).Lvl = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "STATS", "ELV")
        Cuenta.Chars(i).Oro = GetVar(CharPath & Cuenta.Chars(i).name & ".chr", "STATS", "GLD")
        Paquete = Paquete & _
        i & "," & Cuenta.Chars(i).name & "," & Cuenta.Chars(i).Head & "," & Cuenta.Chars(i).Body _
        & "," & Cuenta.Chars(i).Helmet & "," & Cuenta.Chars(i).Shield & "," & Cuenta.Chars(i).Weapon _
        & "," & Cuenta.Chars(i).Status & "," & Cuenta.Chars(i).Pos & "," & Cuenta.Chars(i).Oro _
        & "," & Cuenta.Chars(i).Lvl & "~"
        DoEvents
    Next i
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "NUMPJS" & Cuenta.NumChars)
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "PJ" & Paquete)
Else
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "NUMPJS" & 0)
    Exit Sub
End If
...
'    If Nombre(i) = "" Then 'sin nombre, borrado
'        For j = i To NumPjs - i
'            Call WriteVar(CuentasPath & name & ".ct", _
'            "INIT", "PJ" & j, _
'            GetVar(CuentasPath & name & ".ct", "INIT", _
'            "PJ" & j + 1))

'       Next j
'    End If
' no se si va este o el de abahjo coco cuenta
'Next i

'For i = 1 To NumPjs
'    If GetVar(CuentasPath & name & ".ct", "INIT", "PJ" & i) = "" Then
'        For j = i To NumPjs ' - i ?
'            If j < NumPjs Then
'                Call WriteVar(CuentasPath & name & ".ct", "INIT", "PJ" & j, GetVar(CuentasPath & name & ".ct", "INIT", "PJ" & j + 1))
'            End If
'        Next j
'    End If
'Next i
...
Sub ConnectNewUser(UserIndex As Integer, name As String, Password As String, UserRaza As String, UserSexo As String, UserClase As String, _
                    ...
                    US21 As String, UserEmail As String, Hogar As String, Cuenta As String, Head As Integer)
...
'Call WriteVar(CuentasPath & UserList(UserIndex).NombreCuenta & ".ct", _
'"INIT", "NumPjs", NumeroPjs + 1)

'Call WriteVar(CuentasPath & UserList(UserIndex).NombreCuenta & ".ct", _
'"INIT", "PJ" & NumeroPjs + 1, UserList(UserIndex).Name)
...
Select Case UserClase
    Case "Paladin"
        UserList(UserIndex).Invent.Object(3).ObjIndex = 987
    Case "Mago"
        UserList(UserIndex).Invent.Object(3).ObjIndex = 988
    Case "Asesino"
        UserList(UserIndex).Invent.Object(3).ObjIndex = 460
    Case "Guerrero"
        UserList(UserIndex).Invent.Object(3).ObjIndex = 989
    Case "Cazador"
        UserList(UserIndex).Invent.Object(3).ObjIndex = 1206
End Select
...
Select Case UserClase
    Case "Cazador"
        UserList(UserIndex).Invent.Object(6).ObjIndex = 1210
...
UserList(UserIndex).NombreCuenta = Cuenta
Dim NumeroPjs As Integer
NumeroPjs = val(GetVar(App.Path & "\Cuentas\" & Cuenta & ".ct", "INIT", "NumPjs"))

Call WriteVar(App.Path & "\Cuentas\" & UserList(UserIndex).NombreCuenta & ".ct", _
"INIT", "NumPjs", NumeroPjs + 1)

Call WriteVar(App.Path & "\Cuentas\" & Cuenta & ".ct", _
"INIT", "PJ" & NumeroPjs + 1, UserList(UserIndex).name)
...
If UserList(UserIndex).flags.Navegando = 1 Then Call SendData(SendTarget.ToIndex, UserIndex, 0, "NG")
...
ElseIf Niebla = 1 Then
Call SendData(ToIndex, UserIndex, 0, "NIE" & 1)
End If
    ...
    If Not UserList(UserIndex).flags.UserLogged Then
        ...
        Select Case Left$(rData, 6)
                ...
                For i = 1 To val(GetVar(Archivo, "INIT", "NumPjs"))
                    ...
                    If UCase$(GetVar(Archivo, "INIT", "PJ" & i)) = UCase$(rData) Then 'encontre mi user ^^
                    
                        Debug.Print "encontre im user, se llama " & rData
    ...
    Exit Sub
        ...
        Case "XIKLON"
                rData = Right$(rData, Len(rData) - 6)
                    ...
                    Pass11 = ReadField(1, rData, 44)
                    tName = ReadField(2, rData, 44)
                    
                    SegCode = ReadField(3, rData, 44)
                    
                    If SegCode <> ClaveSeguridad Then
                    ...
                    Call ConnectCuenta(UserIndex, tName, Pass11)
                Exit Sub
            ...
            Case "NLOGIN"
                ...
                rData = Right$(rData, Len(rData) - 6)
                    ...
                    Call ConnectNewUser(UserIndex, ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(3, rData, 44), ReadField(4, rData, 44), ReadField(5, rData, 44), ReadField(6, rData, 44), _
                                        ...
                                        ReadField(25, rData, 44), ReadField(26, rData, 44), ReadField(27, rData, 44), ReadField(28, rData, 44), ReadField(29, rData, 44), ReadField(30, rData, 44)) 'coco
                
                '    MsgBox ReadField(28, rData, 44) & ReadField(29, rData, 44) & ReadField(30, rData, 44)
...
If UCase$(Left$(rData, 7)) = "/TELEP " Then
   ...
   If tIndex <= 0 Then
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Usuario offline. Sus coordenadas han sido cambiadas a las elegidas." & FONTTYPE_INFO)
...
'IR A
If UCase$(Left$(rData, 5)) = "/IRA " And UserList(UserIndex).flags.Privilegios > PlayerType.User Then
    rData = Right$(rData, Len(rData) - 5)
...
If UCase$(Left$(rData, 5)) = "/BAN " Then
    ...
    If tStr = "Erwin" Then
Call SendData(ToIndex, UserIndex, 0, "||Erwin no puede ser banneado." & FONTTYPE_INFO)
Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).name & " intento banear a ERWIN" & FONTTYPE_INFO)
Exit Sub
End If
...
If UCase$(rData) = "/CERRARTORNEO" Then
    If Hay_Torneo = True Then
        Call SendData(SendTarget.ToAll, 0, 0, "||Las Inscripciones para el Torneo fueron cerradas !!" & FONTTYPE_CELESTE_NEGRITA)
        Hay_Torneo = False
        Torneo.Reset
    End If
    Exit Sub
...
If UCase$(Left$(rData, 9)) = "/INVASION" Then
    Dim NoichInvasion As Integer
    Dim NoichNpcsInvasion As WorldPos
    NoichNpcsInvasion.Map = 1
    NoichNpcsInvasion.X = 50
    NoichNpcsInvasion.Y = 50
        For NoichInvasion = 1 To 9
             Call SpawnNpc(925, NoichNpcsInvasion, True, False)
        Next NoichInvasion
Exit Sub
End If

If UCase$(Left$(rData, 9)) = "/POLLOS" Then
    Dim Noichpollos As Integer
    Dim NoichNpcspollos As WorldPos
    NoichNpcsInvasion.Map = 137
    NoichNpcsInvasion.X = 43
    NoichNpcsInvasion.Y = 39
        For NoichInvasion = 1 To 999
             Call SpawnNpc(506, NoichNpcsInvasion, True, False)
        Next NoichInvasion
Exit Sub
End If
...
uName = ReadField(2, rData, 32)
uName = Replace(uName, "+", " ")
otroIndex = NameIndex(uName)
Debug.Print "OTROINDEX:" & otroIndex
If otroIndex = 0 Or otroIndex = UserIndex Then Exit Sub
msg = mid$(rData, 8 + Len(uName) + 1, Len(rData) - 8 + Len(uName) + 1)
Debug.Print msg
Call SendData(SendTarget.ToIndex, otroIndex, 0, "||Mensaje de " & UserList(UserIndex).name & ": " & msg & FONTTYPE_INFO)
Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Mensaje entregado." & FONTTYPE_INFO)
Exit Sub
...
'Standelf
'Dia, Tarde, Noche, Amanecer, Niebla
